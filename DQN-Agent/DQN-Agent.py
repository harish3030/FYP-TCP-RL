...
for e in range(iterationNum):
    ...
    running_rew=0
    running_rtt=0
    cnt_tp=[0 for i in range(leafs)]
    cnt_rew=[0 for i in range(leafs)]
    cnt_rtt=[0 for i in range(leafs)]
    avg_tp=[0 for i in range(leafs)]
    avg_rew=[0 for i in range(leafs)]
    avg_rtt=[0 for i in range(leafs)]
    old_U=0
...
# plot_graphs(global_tp_history,'Avg Throughput over time','Throughput (Mbits/s)',iterationNum)
# plot_graphs(global_rtt_history,'Avg RTT over time','RTT (seconds)',iterationNum)
# plot_graphs(global_rew_history,'Avg Reward sum plot','Accumulated reward',iterationNum)
# print(tp)       
# mpl.rcdefaults()
# mpl.rcParams.update({'font.size': 16})
